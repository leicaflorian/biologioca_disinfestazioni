      ...
      <div class="col">
        <div class="card">
          <div class="card-header">
            ...
            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/services/create') }}"
               role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service.actions.create') }}</a>
          </div>
            ...
            <div class="card-block">
              ...
              <table class="table table-hover table-listing">
                ...
                <tr>
                  ...
                  {{--                                        <th is='sortable' :column="'description'">{{ trans('admin.service.columns.description') }}</th>--}}
                  {{--                                        <th is='sortable' :column="'description_long'">{{ trans('admin.service.columns.description_long') }}</th>--}}
                  {{--                                        <th is='sortable' :column="'description_short'">{{ trans('admin.service.columns.description_short') }}</th>--}}
                  {{--                                        <th is='sortable' :column="'img_cover'">{{ trans('admin.service.columns.img_cover') }}</th>--}}
                  {{--                                        <th is='sortable' :column="'img_cover_low_res'">{{ trans('admin.service.columns.img_cover_low_res') }}</th>--}}
                  {{--                  <th is='sortable' :column="'img_cover_alt'">{{ trans('admin.service.columns.img_cover_alt') }}</th>--}}
                  <th is='sortable' :column="'has_page'">{{ trans('admin.service.columns.has_page') }}</th>
                  <th is='sortable' :column="'featured'">{{ trans('admin.service.columns.featured') }}</th>
                  <th is='sortable' :column="'order'">{{ trans('admin.service.columns.order') }}</th>
                ...
                <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                  ...
                  {{--                  <td>@{{ item.img_cover_low_res }}</td>--}}
                  {{--                  <td>@{{ item.img_cover_alt }}</td>--}}
                  <td>
                    <label class="switch switch-3d switch-success">
                      <input type="checkbox" class="switch-input" v-model="collection[index].has_page"
                             @change="toggleSwitch(item.resource_url, 'has_page', collection[index])">
                      <span class="switch-slider"></span>
                    </label>
                  </td>
                  <td>
                    <label class="switch switch-3d switch-success">
                      <input type="checkbox" class="switch-input" v-model="collection[index].featured"
                  ...
                  <td>
                    <div class="row no-gutters">
                      <div class="col-auto">
                        <a class="btn btn-sm btn-warning" :href="'/servizi/' + item.slug"
                           title="{{ trans('brackets/admin-ui::admin.btn.view') }}" role="button" target="_blank">
              ...
              <div class="no-items-found" v-if="!collection.length > 0">
                ...
                <a class="btn btn-primary btn-spinner" href="{{ url('admin/services/create') }}" role="button"><i
