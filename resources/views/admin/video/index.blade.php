...
@section('title', trans('admin.video.actions.index'))
      ...
      <div class="col">
        <div class="card">
          <div class="card-header">
            <i class="fa fa-align-justify"></i> {{ trans('admin.video.actions.index') }}
            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/videos/create') }}"
               role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.video.actions.create') }}</a>
          </div>
            ...
            <div class="card-block">
              ...
              <table class="table table-hover table-listing">
                ...
                <tr>
                  ...
                  <th is='sortable' :column="'id'">{{ trans('admin.video.columns.id') }}</th>
                  <th is='sortable' :column="'video_link'">{{ trans('admin.video.columns.video_link') }}</th>
                  <th is='sortable' :column="'service_id'">{{ trans('admin.video.columns.service_id') }}</th>
                  <th is='sortable' :column="'order'">{{ trans('admin.video.columns.order') }}</th>
                ...
                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                  <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                  ...
                                                  @click="onBulkItemsClickedAll('/admin/videos')"
                    ...
                    <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/videos/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                ...
                <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                  ...
                  <td>@{{ item.id }}</td>
                  <td><img :src="item.thumb_default" class="img-thumbnail"></td>
                  <td><a :href="`/admin/services/${item.service_id}/edit`">@{{ item.service.title }}</a></td>
                  <td>@{{ item.order }}</td>
              ...
              <div class="no-items-found" v-if="!collection.length > 0">
                ...
                <a class="btn btn-primary btn-spinner" href="{{ url('admin/videos/create') }}" role="button"><i
                      class="fa fa-plus"></i>&nbsp; {{ trans('admin.video.actions.create') }}</a>
              </div>
            </div>
          </div>
        </div>
